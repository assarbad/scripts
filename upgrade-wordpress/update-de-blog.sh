...
function blog_update
{
	...
	if ( \
		...
		if ( set -x; tar -C "$BASEDIR" -czf "$OLDWPBKUP" "${BLOGBASE#$BASEDIR/}" "${WPCONFIG#$BASEDIR/}" $(cd "$BASEDIR" && find -type f -maxdepth 1 -name 'update-blog.sh' -o -name 'update-*-blog.sh' -o -name 'update-wp-*' -o -name '.update-blog.*' -o -name '.update-wp-*') "${OLDDBBKUP##*/}"{,.SHA256SUM} && ( cd "${OLDWPBKUP%/*}" && sha256sum "${OLDWPBKUP##*/}" )|tee "$OLDWPBKUP.SHA256SUM" ); then
			if [[ "$UPDTGZ" != "--backup" ]] && [[ "$UPDTGZ" != "-b" ]]; then
