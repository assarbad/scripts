...
apt-get -f autoremove command-not-found command-not-found-data cloud-init cloud-utils dosfstools e2fslibs e2fsprogs euca2ools friendly-recovery laptop-detect ntfs-3g openssh-client openssh-server parted pciutils unattended-upgrades update-motd wpasupplicant wireless-tools ureadahead tasksel tasksel-data ssh-import lshw iptables update-manager
aptitude purge $(dpkg -l|awk '$1 ~ /^rc/ {print $2}')
rm -rf /root/.aptitude /root/.debtags /etc/
rm -f /root/.viminfo
apt-get -y clean
apt-get -y autoclean
aptitude -y clean
