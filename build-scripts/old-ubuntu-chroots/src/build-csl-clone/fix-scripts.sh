...
FNR==1 {
	...
	print "\t\t*/obj/binutils-src-* | */obj/gdb-src-*)"
	...
	print "\tesac"
	print "\tcase \"\$(pwd)\" in"
	print "\t\t# This makes sure zlib is only available as static lib in that folder"
	print "\t\t*/obj/zlib-first-*)"
	print "\t\t\tset -- \"\$@\" \"--static\""
	print "\t\t\t;;"
	print "\t\t*)"
	print "\t\t\t;;"
	...
	print "\tcase \"\$(pwd)\" in"
	print "\t\t*/obj/binutils-*)"
	print "\t\t\t( set -x; pwd; env LEXLIB=\"-l:libfl.a\" make \"\$@\" )"
	print "\t\t\t;;"
	print "\t\t*)"
...
END {
	...
	print  "find \"\${SCRATCH}/\${TCRELEASE}/install\" -type f -exec file {} +|grep ELF|grep x86-64|cut -d : -f 1|while read fname; do ( set -x; ldd \$fname ); done 2>&1|tee \$CURRABSPATH/build-host-binaries.txt"
}
EOF
)
