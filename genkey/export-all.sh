...
while read -r KEYID REMAINDER; do
	...
	(
		IDENTITY="${REMAINDER##*<}";
		IDENTITY="${IDENTITY%%>*}";
		SECFILENAME="0x${KEYID:24}_$IDENTITY.sec.asc";
		PUBFILENAME="0x${KEYID:24}_$IDENTITY.pub.asc";
		gpg --export-secret-keys -ao "$SECFILENAME" "$KEYID"
		gpg --export -ao "$PUBFILENAME" "$KEYID"
	)
done
