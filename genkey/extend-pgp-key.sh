...
[[ -n "$DEBUG" ]] && set -x
declare -r BASEDATE='2023-12-31 23:59:59Z'
declare -r DURATION="4y"
gpg -Kv --with-subkey-fingerprints --batch 2> /dev/null \
	| awk '$1 == "sec" { getline; KID=$1; next; } $1 == "uid" && $3 ~ /expired/ { printf("%s", KID); DELIM=" "; for (f=2; f<=NF; ++f) { printf("%s%s", DELIM, $f); }; printf("\n") }' \
	| \
